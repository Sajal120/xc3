...
def get_latest_cost_and_usage_report(bucket_name, report_prefix):
    """
    Gets the latest Cost and Usage Report from an S3 bucket based on the timestamp in the object key.
...
def classify_ec2_category(row):
    """
    Classify the service category for rows with 'Amazon Elastic Compute Cloud' based on the 'lineItem/ResourceId'.
...
def extract_cost_by_service_and_resource(data):
    relevant_columns = ['product/region', 'product/ProductName', 'lineItem/ResourceId', 'lineItem/UnblendedCost']
    
    df = data[relevant_columns]
    df['ServiceCategory'] = df.apply(classify_ec2_category, axis=1)

    # Get the top 5 services for each region
    top_services_df = df.groupby(['product/region', 'ServiceCategory'])['lineItem/UnblendedCost'].sum().groupby('product/region', group_keys=False).nlargest(5).reset_index()
    ...
    grouped_df = df_filtered.groupby(['product/region', 'ServiceCategory', 'lineItem/ResourceId'], as_index=False)['lineItem/UnblendedCost'].sum()
...
def create_and_push_gauge(data):
    try:
        ...
        gauge_resources_cost = Gauge('resources_cost',
        ...
        for index, row in data.iterrows():
            ...
            gauge_resources_cost.labels(region=region, service=service, resource =resource_id).set(float(cost))
            
            # add the dictionary to the list
            data_dict = {"Service": service, "Region": region,"ResourceId":resource_id, "Cost": cost}
            data_list.append(data_dict)

        # Push the metric to the Prometheus Gateway
        push_to_gateway(os.environ["prometheus_ip"], job='resources_cost', registry=registry)
