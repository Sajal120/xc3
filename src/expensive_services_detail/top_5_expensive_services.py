...
def extract_cost_by_service_and_region(data):
    ...
    grouped_df = df.groupby(['product/region','ServiceCategory'])['lineItem/UnblendedCost'].sum().reset_index()
    # Get the top N most expensive services for each region
    top_services_df = grouped_df.groupby(['product/region']).apply(lambda x: x.nlargest(5, 'lineItem/UnblendedCost')).reset_index(drop=True)
...
def create_and_push_gauge(data):
    try:    
        ...
        gauge_top_service_cost = Gauge('top_service_cost',
        ...
        for index, row in data.iterrows():
            ...
            gauge_top_service_cost.labels(region=region, service=service).set(float(cost))
        ...
        push_to_gateway(os.environ["prometheus_ip"], job='services_cost', registry=registry)
...
def push_to_s3_bucket(json_data):
    # upload JSON file to S3 bucket
        key_name = f'{os.environ["top5_expensive_service_prefix"]}/top5_expensive_service.json'
        try:
